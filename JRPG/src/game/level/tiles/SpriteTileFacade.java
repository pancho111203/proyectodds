...
public class SpriteTileFacade {
	...
	// en esta clase tenemos toda la funcionalidad que hace falta por ahora respecto a tiles y sprites
	...
	public void initSpriteOnTileOnHex(String name, int x, int y, Spritesheet sheet,int h ){
		// metodo combo que inicializa un sprite y lo coloca en un tile(con el mismo nombre),
	...
	public void initTileOnHex(int h, String name, String sprite, boolean animation){ 
		...
		// si animation es true, se carga una animacion (SI NO EXISTE DEVUELVE NULL)
		Sprite s;
		if(!animation){
			//load SingleSprite
	...
	public void initHex(int h, String name){ // añade un tile creado previamente a el hex
		lt.addHex(h, name);
	}
